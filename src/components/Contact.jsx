...
//           action="https://formspree.io/f/xwprkwww"
//           method="post"
...
const Contact = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch("https://formspree.io/f/xwprkwww", {
        method: "POST",
        headers: {
  ...
  return (
    <>
      ...
      <motion.p variants={fadeIn('', '', 0.15, 1)} className={style.subtitle}>
        ...
        Don&apos;t hesitate to reach out and let&apos;s make something great.
      </motion.p>
      <div className={style.container}>
        <motion.form
          variants={slideIn('left', '', 0, 1)}
        ...
        >
          <div className={style.form}>
            ...
            {status === "SUCCESS" && (
              <p className={style.success}>✅ Thanks! Your message has been sent.</p>
            )}
            {status === "ERROR" && (
              <p className={style.error}>❌ Oops! Something went wrong. Please try again.</p>
            )}
          </div>
        </motion.form>
